...
const effectVideoChain = (effects = []) => {
...
const seconds = (n) => (typeof n === "number" ? n : parseFloat(n || 0) || 0).toFixed(3);

/**
 * Converts timeline data to FFmpeg command arguments
 */
function buildFfmpegFromTimeline(timelineData) {
  ...
  const inputs = []; // array of `-i <path or lavfi>`
  const filters = []; // strings composing filter_complex
  const videoLayers = []; // collect labeled video streams to overlay sequentially
  const audioLayers = []; // collect labeled audio streams to mix
  ...
  const audioElements = mediaElements.filter(
    (item) => 
      ...
      (item.trackType === "media" && item.mediaItem?.type === "video" && !(item.element.muted))
  );
  
  const audioClipLabels = [];
...
module.exports = { buildFfmpegFromTimeline };
